...
export const useDailyNoDIYStore = defineStore("dailyNoDIY", () => {
  // 記錄每天是否成功不自己動手 (true = 成功, false = 失敗)
  const dailyNoDIYResults = ref<Record<string, boolean>>(useMockData ? generateMockData() : {});
  
  // 計算當前總分 - 從所有結果計算
  const totalScore = computed(() => {
    return calculateTotalScore(dailyNoDIYResults.value);
  });
  ...
  const recordResult = (success: boolean) => {
    const today = getTodayKey();
    dailyNoDIYResults.value[today] = success;
    // 不需要手動更新分數，因為它是計算屬性
  };
  ...
  const getResultByDate = (date: Date) => {
    const dateKey = formatDateToKey(date);
    return dailyNoDIYResults.value[dateKey];
  };
  ...
  const getScoreByDate = (targetDate: Date) => {
    ...
    // 獲取所有日期並排序
    const dates = Object.keys(dailyNoDIYResults.value).sort();
    ...
    for (const dateKey of dates) {
      ...
      // 累加分數
      const success = dailyNoDIYResults.value[dateKey];
