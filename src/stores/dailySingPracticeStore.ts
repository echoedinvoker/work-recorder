...
export const useDailySingPracticeStore = defineStore("dailySingPractice", () => {
  // 記錄每天是否完成唱歌練習 (true = 成功, false = 失敗)
  const dailySingPracticeResults = ref<Record<string, boolean>>(useMockData ? generateMockData() : {});
  
  // 計算當前總分 - 從所有結果計算
  const totalScore = computed(() => {
    return calculateTotalScore(dailySingPracticeResults.value);
  });
  ...
  const recordResult = (success: boolean) => {
    ...
    dailySingPracticeResults.value[today] = success;
    // 不需要手動更新分數，因為它是計算屬性
  };
  ...
  const getResultByDate = (date: Date) => {
    const dateKey = formatDateToKey(date);
    return dailySingPracticeResults.value[dateKey];
  };
  ...
  const clearAllHistory = () => {
    dailySingPracticeResults.value = {};
  ...
  const getScoreByDate = (targetDate: Date) => {
    ...
    // 獲取所有日期並排序
    const dates = Object.keys(dailySingPracticeResults.value).sort();
    ...
    for (const dateKey of dates) {
      ...
      // 累加分數
      const success = dailySingPracticeResults.value[dateKey];
