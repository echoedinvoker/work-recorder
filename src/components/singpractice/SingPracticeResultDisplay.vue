<template>
  <div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-lg font-semibold text-gray-800 mb-4">唱歌練習統計</h3>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="bg-red-50 p-4 rounded-lg">
        <div class="text-sm text-gray-600">未完成次數</div>
...
const successCount = computed(() => {
  return Object.values(store.dailySingPracticeResults).filter(result => result === true).length;
});

// 計算失敗次數
const failureCount = computed(() => {
  return Object.values(store.dailySingPracticeResults).filter(result => result === false).length;
});
