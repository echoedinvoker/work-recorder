<template>
  <div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-lg font-semibold text-gray-800 mb-4">不自己動手統計</h3>
...
const successCount = computed(() => {
  return Object.values(store.dailyNoDIYResults).filter(result => result === true).length;
});

// 計算失敗次數
const failureCount = computed(() => {
  return Object.values(store.dailyNoDIYResults).filter(result => result === false).length;
});
