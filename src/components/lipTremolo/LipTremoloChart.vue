<template>
  ...
  >
    ...
    <div class="flex justify-between text-sm text-gray-600 pt-4 border-t">
      <div>
        <span class="text-gray-500">平均次數:</span>
        <span class="font-medium ml-1">{{ avgScore }}次</span>
      </div>
      <div>
        <span class="text-gray-500">最高{{ currentPeriod === 'day' ? '次數' : '累積' }}:</span>
        <span class="font-medium ml-1">{{ maxScore }}次</span>
      </div>
    </div>
  </div>
</template>
...
const storeAdapter = {
  getScoreByDate: (date: Date) => store.getCountByDate(date),
  UNIT: store.UNIT
};
